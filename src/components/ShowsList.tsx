  ...
  return (
    <>
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuDm8rGeAKDi7Qp9opVj8exc849kJ07VR_-gisO04g_8OOWlscyQCy62TBdjTSmmI2F2oiy249i_u8rD1ulSVZckfuxjcBYzri_I56ygkwkeBuZAILo9IzU4u0cFNHK20e-M0i8sED6QqSgMYjOZzGrV78XqteC0XbbuIdlH4Fei7RXp8quLZakzli6ziu9Yv3y19Zyq_fzv9oeu5cCsLHmyEAA6wHLbEYyPTLt4-ngtUQunoSN7MCgRqN3tONv9ZsAzERePqLyIz9Q')" }}></div>
      ...
      <div className="relative z-10 pt-24 pb-48 px-6 md:px-16 md:pr-32 max-w-7xl mx-auto min-h-screen flex flex-col">
        <header className="mb-16">
          <h1 className="text-5xl md:text-7xl text-[#e5e2e1] uppercase mb-4 opacity-90 font-bold tracking-tighter">Archive Directory</h1>
          <p className="text-base md:text-lg text-[#e9bcba] max-w-xl">A chronological record of aural decimation. Raw data extracted from the touring history.</p>
        </header>

        <div className="flex flex-col gap-0 w-full">
          <div className="flex items-center py-4 px-4 border-b border-white/10 opacity-50 mb-2">
            <div className="w-1/4 md:w-1/5 text-xs tracking-widest text-[#e9bcba] uppercase font-mono">Date</div>
            <div className="w-1/2 md:w-3/5 text-xs tracking-widest text-[#e9bcba] uppercase font-mono">Show</div>
          ...
          {events.map((event, idx) => (
            <div key={idx} className="group flex flex-col border-b border-white/5 hover:bg-[#2a2a2a]/40 transition-colors duration-300 relative overflow-hidden">
              ...
              <div className="flex items-center py-6 px-4 relative z-10">
                <div className="w-1/4 md:w-1/5 text-sm tracking-wider text-[#929090] uppercase font-mono">{formatDateList(event.startDate)}</div>
                <div className="w-1/2 md:w-3/5 flex flex-col justify-center">
                  <div className="text-2xl md:text-4xl text-[#e5e2e1] group-hover:text-[#ffb3b2] transition-colors duration-300 uppercase tracking-tight font-bold">
                    {event.name}
                  </div>
                  <span className="text-xs tracking-wider text-[#e9bcba] uppercase font-mono opacity-80 mt-1">
                    {[...event.performer].reverse().map(p => p.name).join(' • ')}
                  </span>
                </div>
                <div className="w-1/4 md:w-1/5 text-right text-sm tracking-wider text-[#ffb3b2] uppercase font-mono">
                  {event.location?.name}
                  <br /><span className="text-[#e9bcba] text-[10px] opacity-70 uppercase">{event.location?.address?.addressLocality || 'UNKNOWN'}</span>
                </div>
              </div>
            </div>
          ))}
    ...
    <>
      ...
      <div className="relative z-10 pt-24 pb-48 px-6 md:px-16 md:pr-32 max-w-7xl mx-auto min-h-screen">
        <header className="mb-16 border-b border-white/10 pb-8">
          <h1 className="text-5xl md:text-7xl font-bold text-[#e5e2e1] mb-2 tracking-tighter uppercase">Visual Archive</h1>
          <p className="text-base md:text-lg text-[#e9bcba] max-w-xl">A curated collection of brutalist tour graphics from the underground circuit. High contrast. High distortion.</p>
        </header>
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-8 space-y-8">
          {events.map((event, idx) => {
            ...
            return (
              <article key={idx} className="poster-card relative break-inside-avoid overflow-hidden bg-[#201f1f] cursor-pointer rounded-md group">
                {/* Image now determines the block's height adaptively */}
                <img alt={event.name} className="w-full h-auto object-cover grayscale mix-blend-luminosity opacity-80 group-hover:grayscale-0 transition-all duration-500 block" src={imgUrl} />

                {/* Hover Overlay: hidden by default, visible on hover with a darker red background */}
                <div className="absolute inset-0 bg-[#bf002a] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 z-10">
                  <h3 className="text-2xl font-bold text-white uppercase leading-tight">{event.name}</h3>
                  <div className="flex flex-col gap-1 mt-1">
                    ...
                    <span className="text-xs tracking-widest text-white uppercase font-mono">
                      {formatDateList(event.startDate)} // {event.location?.address?.addressLocality || event.location?.name}
                    </span>
                  </div>
                </div>
              </article>
  ...
  return (
    <div className="relative min-h-screen w-full font-sans overflow-x-hidden">
      ...
      <aside className="fixed right-0 top-0 h-full flex-col justify-center items-center gap-6 z-50 bg-[#0e0e0e]/60 backdrop-blur-md border-l border-white/5 w-16 hidden md:flex">
        <button onClick={() => setViewMode('list')} className={`p-3 rounded-full transition-all group cursor-pointer ${viewMode === 'list' ? 'text-[#ffb3b2] bg-[#ffb3b2]/10' : 'text-[#e9bcba] hover:bg-[#ffb3b2]/10 hover:text-[#ffb3b2]'}`} title="List View">
          <List className="w-6 h-6" />
        </button>
        <button onClick={() => setViewMode('poster')} className={`p-3 rounded-full transition-all group cursor-pointer ${viewMode === 'poster' ? 'text-[#ffb3b2] bg-[#ffb3b2]/10' : 'text-[#e9bcba] hover:bg-[#ffb3b2]/10 hover:text-[#ffb3b2]'}`} title="Poster View">
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full z-50 bg-[#0e0e0e]/90 backdrop-blur-xl border-t border-white/10 flex justify-around items-center py-4">
        <button onClick={() => setViewMode('list')} className={`flex flex-col items-center gap-1 ${viewMode === 'list' ? 'text-[#ffb3b2]' : 'text-[#e9bcba]'}`}>
          <List className="w-5 h-5" />
          <span className="text-[10px] uppercase tracking-wider font-mono">List</span>
        </button>
        <button onClick={() => setViewMode('poster')} className={`flex flex-col items-center gap-1 ${viewMode === 'poster' ? 'text-[#ffb3b2]' : 'text-[#e9bcba]'}`}>
          <LayoutGrid className="w-5 h-5" />
          <span className="text-[10px] uppercase tracking-wider font-mono">Poster</span>
        </button>
      </div>
