...
const getInternalPackagesCacheDir = () => path.join(process.cwd(), `.cache/internal-packages`);
...
function installPrebuiltLmdb() {
  ...
  let packageJson;
  try {
    const modulePath = path.dirname(require.resolve(`lmdb`)).replace(`/dist`, ``);
  ...
  if (!Object.keys(optionalDependencies).find(p => p === lmdbPackage)) return false;
  try {
    const lmdbRequire = _module.default.createRequire(require.resolve(`lmdb`));
...
async function installIfMissingLmdb() {
  ...
  await (0, _execa.default)(`npm`, [`install`, ...npmAdditionalCliArgs, `${lmdbPackage}@${_package.dependencies.lmdb}`], options);
...
async function createGraphqlEngineBundle(rootDir, reporter, isVerbose) {
  ...
  if (_redux.store.getState().adapter.instance) {
